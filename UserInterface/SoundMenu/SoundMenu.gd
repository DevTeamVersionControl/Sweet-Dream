...
func set_volume(_dummy):
	AudioServer.set_bus_volume_db(0, linear_to_db(float(master_volume.value)/100))
	AudioServer.set_bus_volume_db(1, linear_to_db(float(music_volume.value)/100))
	AudioServer.set_bus_volume_db(2, linear_to_db(float(effects_volume.value)/100))
