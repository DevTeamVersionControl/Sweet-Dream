...
func physics_update(delta: float) -> void:
	...
	# TODOConverter40 looks that snap in Godot 4.0 is float, not vector like in Godot 3 - previous value `Vector2.DOWN * 16`
	player.set_up_direction(Vector2.UP)
	player.set_floor_stop_on_slope_enabled(false)
	player.set_max_slides(4)
	player.set_floor_max_angle(PI/4)
