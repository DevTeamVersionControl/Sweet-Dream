...
func enter(_msg := {}) -> void:
	...
	await candy_corn.animation_player.animation_finished
	if candy_corn.health > 0:
...
func physics_update(delta):
	...
	candy_corn.set_velocity(candy_corn.motion)
	candy_corn.move_and_slide()
	candy_corn.motion = candy_corn.velocity
